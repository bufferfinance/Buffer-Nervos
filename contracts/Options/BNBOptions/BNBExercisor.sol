 ...
 */
contract BNBExercisor is Ownable {
    /**
     * @notice Check if the sender can exercise an active option
     * @param optionID ID of your option
     */
    function isExercisable(uint256 optionID, BufferBNBOptions optionsContract)
        external
        view
    {
        address tokenOwner = optionsContract.ownerOf(optionID);
        bool isAutoExerciseTrue = optionsContract.autoExerciseStatus(
            tokenOwner
        ) && msg.sender == owner();
     ...
     */
    function isUnlockable(uint256 optionID, BufferBNBOptions optionsContract)
        external
        view
    {
        (
        ...
        require(expiration < block.timestamp, "Option has not expired yet");
     ...
     */
    function calculateProfit(uint256 optionID, BufferBNBOptions optionsContract)
        external
        view
        returns (uint256 profit)
    {
        ...
        (, int256 latestPrice, , , ) = optionsContract
            .priceProvider()
            .latestRoundData();
        ...
        if (optionType == IBufferOptions.OptionType.Call) {
            ...
            profit = ((currentPrice - strike) * amount) / currentPrice;
        } else {
            require(strike >= currentPrice, "Current price is too high");
            profit = ((strike - currentPrice) * amount) / currentPrice;
        }
